...
static const float32 TARGET_ANGLE_DEGREE = 10; //단위 : 도
static const float32 TARGET_ANGLE_RADIAN = 10.0f * (IFX_PI/180); //단위 : 라디안
static const float32 ANGULAR_VELOCITY_DEGREE = 1.6f; //단위 : 도/초
static const float32 ANGULAR_VELOCITY_RADIAN = 1.6f * (IFX_PI/180); //단위 : 라디안/초

uint32 rotation_time_T1_ticks;
float32 turn_radius = 0;

float32 rotation_time_T1;
float32 travel_time_T2;
...
void T1_rotation_Right(void)
{
    ...
    rotation_time_T1 = TARGET_ANGLE_RADIAN / ANGULAR_VELOCITY_RADIAN; //단위 : 초

    //FCA 기능 정지
    IfxStm_disableComparatorInterrupt(&MODULE_STM1, IfxStm_Comparator_0);

    turn_radius = set_turn(Right, ANGULAR_VELOCITY_RADIAN);
...
void T2_straight(void)
{
    ...
    travel_time_T2 = calculate_T2_time();
...
void T3_rotation_left(void)
{
    ...
    init_lc_T3_STM_IT(rotation_time_T3);
}
...
void init_lc_T3_STM_IT(float32 rotation_time_T3)
{
    ...
    uint32 travel_time_T3_ticks = IfxStm_getTicksFromMicroseconds(BSP_DEFAULT_TIMER, (uint32)(rotation_time_T3*1000000));
